...
public class Day10Controller {
    Logger logger = LoggerFactory.getLogger(Day10Controller.class);

    @Autowired
    InputService inputService;

    @GetMapping("/day10/part1/{realm}")
    public long execute1(@PathVariable Realm realm, @RequestParam Optional<String> override) throws IOException {
        String input = inputService.getInput(10, realm, override);
        var topoMap = TopoMap.fromString(input);
        return topoMap.score();
    }

    @GetMapping("/day10/part2/{realm}")
    public long execute2(@PathVariable Realm realm, @RequestParam Optional<String> override) throws IOException {
        String input = inputService.getInput(10, realm, override);
        var topoMap = TopoMap.fromString(input);
        return topoMap.score2();
    }
}